    ...
    public Accounts getAccountWithDebitCard(String cardNo) {
        ...
        try {
            Accounts accounts = (Accounts) conn.getEntityManager()
                    .createQuery("from Accounts where debit_card_number=:debit_card_number")
                    .setParameter("debit_card_number", cardNo).getSingleResult();
            return accounts;
        }catch (NoResultException noResultException){
            return null;
        }
    ...
    public void updateUserId(int userId, String creditCard) throws  Exception{
        ...
        try {
            ...
            conn.getEntityManager()
                    .createQuery("update Accounts set user_id=:user_id where debit_card_number=:debit_card_number")
                    .setParameter("user_id", userId).setParameter("debit_card_number", creditCard).executeUpdate();
            txn.commit();
